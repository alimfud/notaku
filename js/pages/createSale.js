...
function resetState() {
  state = { customer: null, lines: [], discountAmount: 0, payment: 0, note: '' };
}
...
function drawLines(root) {
  ...
  if (!state.lines.length) {
    el.innerHTML = `<div style="padding:24px;text-align:center;color:var(--muted);font-size:13.5px;">Belum ada item. Ketuk "Tambah" untuk memilih produk.</div>`;
    return;
  }
...
async function addItem(root) {
  ...
  const result = await openSheet((body, close) => {
    ...
    function drawList(filter) {
      ...
      listEl.querySelectorAll('.row').forEach((r) => {
        r.onclick = () => close(rows.find((p) => p.id === r.dataset.id));
      });
    }
    drawList('');
